...
type ZoneScan = { matches: Array<{ id: number; code: string }>; added: number };

async function api<T>(path: string, method = "GET", body?: unknown): Promise<T> {
  if (path === "/api/sync" && method === "POST") {
    const stocks = new Map<number, Stock>();
    for (const watchlist of await api<Watchlist[]>("/api/watchlists")) for (const stock of await api<Stock[]>(`/api/watchlists/${watchlist.id}/stocks`)) stocks.set(stock.id, stock);
    for (const stock of stocks.values()) await api(`/api/stocks/${stock.id}/sync`, "POST");
    return { ok: true } as T;
  }
...
function WatchlistClearer() {
  ...
  useEffect(() => setTarget(document.querySelector(".watchlist-title > div:last-child")), []);
  const clear = () => {
    const select = document.querySelector(".watchlist-title select") as HTMLSelectElement | null, name = select?.selectedOptions[0]?.text ?? "this watchlist";
    if (!select?.value || !confirm(`Remove every stock from ${name}?`)) return;
    void api(`/api/watchlists/${select.value}/stocks`, "DELETE").then(() => { sessionStorage.setItem("clearedWatchlist", select.value); location.reload(); }).catch((error) => alert(error.message));
  };
  return target ? createPortal(<button type="button" className="quiet-button clear-watchlist" onClick={clear}>Clear all</button>, target) : null;
}
...
function RowRemovers() {
  ...
  useEffect(() => {
    ...
    update(); const observer = new MutationObserver(update); observer.observe(list, { childList: true });
    return () => observer.disconnect();
  }, []);
  const remove = async (row: Element) => {
    const select = document.querySelector(".watchlist-title select") as HTMLSelectElement | null, code = row.querySelector("strong")?.textContent;
    if (!select?.value || !code || !confirm(`Remove ${code} from this watchlist?`)) return;
    try {
      const stock = (await api<Stock[]>(`/api/watchlists/${select.value}/stocks`)).find((item) => item.code === code);
      if (!stock) return;
      await api(`/api/watchlists/${select.value}/stocks/${stock.id}`, "DELETE");
      sessionStorage.setItem("clearedWatchlist", select.value); location.reload();
    } catch (error) { alert((error as Error).message); }
  };
  return <>{rows.filter((row) => row.isConnected).map((row) => createPortal(<span key="remove" role="button" tabIndex={0} className="icon-button remove row-remove" aria-label="Remove stock" onClick={(event) => { event.preventDefault(); event.stopPropagation(); void remove(row); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); event.stopPropagation(); void remove(row); } }}><IconTrash size={16} /></span>, row))}</>;
}

function Chart({ bars, fit, swingMultiplier }: { bars: Bar[]; fit: boolean; swingMultiplier: number }) {
  const ref = useRef<HTMLDivElement>(null);
  const [showZigZag, setShowZigZag] = useState(true), [showFvg, setShowFvg] = useState(true), [showOb, setShowOb] = useState(true), [chartSwingMultiplier, setChartSwingMultiplier] = useState(swingMultiplier), [fvgMinAtr, setFvgMinAtr] = useState(0.5), [fvgLimit, setFvgLimit] = useState(3), [obDisplacementAtr, setObDisplacementAtr] = useState(1.5), [obLimit, setObLimit] = useState(2), [scanWatchlists, setScanWatchlists] = useState<Watchlist[]>([]);
  useEffect(() => { void api<Watchlist[]>("/api/watchlists").then(setScanWatchlists); }, []);
  useEffect(() => {
    ...
    if (showOb) series.attachPrimitive(new PriceZones(orderBlocks(bars, chartSwingMultiplier, obDisplacementAtr, obLimit, fvgMinAtr), bars, "#22c55e", "#ef4444", "38", true));
    if (showZigZag) {
...
function StockNote({ stock, refresh, message }: { stock: Stock | undefined; refresh: () => Promise<void>; message: (value: string) => void }) {
  ...
  return <form className="note-form" key={stock.id} onSubmit={(event) => { event.preventDefault(); const note = new FormData(event.currentTarget).get("note"); void api(`/api/stocks/${stock.id}/note`, "PUT", { note }).then(refresh).then(() => message("Note saved.")).catch((error) => message(error.message)); }}><label><textarea name="note" defaultValue={stock.note ?? ""} maxLength={2000} placeholder="Add a note…" aria-label="Stock note" /></label>{stock.noteUpdatedAt && <small className="note-time">Updated {formatNoteTime(stock.noteUpdatedAt)}</small>}<div><button>Save</button>{stock.note && <button type="button" className="quiet-button" onClick={() => void api(`/api/stocks/${stock.id}/note`, "DELETE").then(refresh).then(() => message("Note removed.")).catch((error) => message(error.message))}>Remove</button>}</div></form>;
}

function AlertsPanel({ stockId, alerts, refresh, message, close }: { stockId: number | undefined; alerts: Alert[]; refresh: () => Promise<void>; message: (value: string) => void; close: () => void }) {
  return <div className="alert-panel-backdrop" role="presentation" onMouseDown={close}><section className="alert-panel" role="dialog" aria-modal="true" aria-labelledby="alerts-title" onMouseDown={(event) => event.stopPropagation()}><header><h2 id="alerts-title">Price alerts</h2><button className="icon-button" aria-label="Close alerts" onClick={close}><IconX size={20} /></button></header><form className="alert-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); void api("/api/alerts", "POST", { stockId, direction: form.get("direction"), target: form.get("target") }).then(() => { event.currentTarget.reset(); return refresh(); }).catch((error) => message(error.message)); }}><select name="direction"><option value="above">crosses above</option><option value="below">crosses below</option></select><input name="target" type="number" min="0.01" step="0.01" placeholder="DKK target" required /><button disabled={!stockId}>Create alert</button></form><section className="alerts">{alerts.length ? alerts.map((alert) => <div key={alert.id}>{alert.code} {alert.direction} {alert.target.toFixed(2)} DKK · {alert.armed ? "armed" : <button className="quiet-button" onClick={() => void api(`/api/alerts/${alert.id}/rearm`, "POST").then(refresh)}>Re-arm</button>}</div>) : <p>No price alerts yet.</p>}</section></section></div>;
}

function Dashboard() {
  const [watchlists, setWatchlists] = useState<Watchlist[]>([]), [watchlistId, setWatchlistId] = useState<number>(), [stocks, setStocks] = useState<Stock[]>([]), [stockId, setStockId] = useState<number>(), [bars, setBars] = useState<Bar[]>([]), [timeframe, setTimeframe] = useState("1d"), [alerts, setAlerts] = useState<Alert[]>([]), [message, setMessage] = useState(""), [syncing, setSyncing] = useState(false), [pickerOpen, setPickerOpen] = useState(false), [marketStocks, setMarketStocks] = useState<MarketStock[]>([]), [marketError, setMarketError] = useState(""), [adding, setAdding] = useState(false), [copyOpen, setCopyOpen] = useState(false), [changeSort, setChangeSort] = useState<"asc" | "desc">(), [symbolSort, setSymbolSort] = useState<"asc" | "desc">(), [sectorSort, setSectorSort] = useState<"asc" | "desc">(), [sectorFilter, setSectorFilter] = useState(""), [stockSearch, setStockSearch] = useState(""), [alertsOpen, setAlertsOpen] = useState(false), [fit, setFit] = useState(false), [swingMultiplier, setSwingMultiplier] = useState(3);
  const loadWatchlists = async () => { const data = await api<Watchlist[]>("/api/watchlists"), cleared = Number(sessionStorage.getItem("clearedWatchlist")); sessionStorage.removeItem("clearedWatchlist"); setWatchlists(data); setWatchlistId((current) => current ?? data.find((watchlist) => watchlist.id === cleared)?.id ?? data[0]?.id); };
  ...
  const filteredStocks = stocks.filter((stock) => (!sectorFilter || stockSector(stock.code) === sectorFilter) && stock.code.toLowerCase().includes(stockSearch.trim().toLowerCase()));
