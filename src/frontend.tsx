import { CandlestickSeries, createChart, CrosshairMode, type Time } from "lightweight-charts";
import { IconChevronDown, IconCopy, IconNote, IconPlus, IconSearch, IconTrash, IconX } from "@tabler/icons-react";
import { useEffect, useRef, useState, type CSSProperties } from "react";
import { createRoot } from "react-dom/client";
import { chartTime } from "./shared";
import "./style.css";
...
function Chart({ bars }: { bars: Bar[] }) {
  ...
  return <div className="chart">{bars.length ? <div ref={ref} /> : <p>No price history for this timeframe.</p>}</div>;
}
...
function Dashboard() {
  ...
  const addStock = async (code: string) => { await api("/api/stocks", "POST", { code, watchlistId }); await loadWatchlists(); if (watchlistId) await loadStocks(watchlistId); setMessage("Stock added to this watchlist."); };
  const openPicker = () => { setPickerOpen(true); if (!marketStocks.length) void loadMarketStocks(); };
  const addFromPicker = (code: string) => { setAdding(code); void addStock(code).then(() => setPickerOpen(false)).catch((error) => setMarketError(error.message)).finally(() => setAdding("")); };
  ...
  return <main className="terminal"><header className="app-header"><div className="app-title"><div><h1>Stock Watcher</h1><p>Denmark stocks · manual sync</p></div><div className="header-actions"><button onClick={openPicker}><IconPlus size={18} /> Add symbol</button><button disabled={syncing} onClick={() => { setSyncing(true); setMessage("Syncing data…"); void api("/api/sync", "POST").then(() => { setMessage("Data synced."); return stockId ? api<{ bars: Bar[] }>(`/api/bars?stock=${stockId}&timeframe=${timeframe}`).then((data) => setBars(data.bars)) : undefined; }).catch((error) => setMessage(error.message)).finally(() => setSyncing(false)); }}>{syncing ? "Syncing…" : "Sync data"}</button></div></div><button className="quiet-button" onClick={() => api("/api/session", "DELETE").then(() => location.reload())}>Sign out</button></header>{message && <p className="message">{message}</p>}<div className="terminal-layout"><aside className="watchlist-panel"><div className="watchlist-title"><label><select value={watchlistId ?? ""} onChange={(event) => setWatchlistId(Number(event.target.value))}>{watchlists.map((watchlist) => <option key={watchlist.id} value={watchlist.id}>{watchlist.name}</option>)}</select><IconChevronDown size={18} /></label><div><button className="icon-button" title="New watchlist" onClick={() => { const name = prompt("Watchlist name"); if (name) void api("/api/watchlists", "POST", { name }).then(loadWatchlists).catch((error) => setMessage(error.message)); }}><IconPlus size={24} /></button><button className="icon-button" title="Copy selected stock" disabled={!stockId || watchlists.length < 2} onClick={() => setCopyOpen((open) => !open)}><IconCopy size={20} /></button><button className="icon-button remove" title="Remove selected stock" disabled={!stockId} onClick={() => void removeStock().catch((error) => setMessage(error.message))}><IconTrash size={20} /></button></div></div>{copyOpen && <form className="copy-form" onSubmit={(event) => { event.preventDefault(); void copyStock(event.currentTarget).catch((error) => setMessage(error.message)); }}><select name="watchlistId">{watchlists.filter((watchlist) => watchlist.id !== watchlistId).map((watchlist) => <option key={watchlist.id} value={watchlist.id}>{watchlist.name}</option>)}</select><button>Copy</button><button type="button" className="quiet-button" onClick={() => setCopyOpen(false)}>Cancel</button></form>}<div className="quote-head"><span>Symbol</span><span>Last</span><span>Chg</span><span>Chg%</span></div><div className="quote-list">{stocks.length ? stocks.map((stock) => <button key={stock.id} title={companyName(stock.code) ?? stock.code} className={`quote-row ${stock.id === stockId ? "selected" : ""}`} onClick={() => setStockId(stock.id)}><span className="quote-symbol"><i style={badge(stock.code)}>{badgeText(stock.code)}</i><strong>{stock.code}</strong>{stock.note && <IconNote className="note-indicator" size={17} aria-label="Has note" />}</span><span>{formatPrice(stock.last)}</span><span className={tone(stock.change)}>{formatChange(stock.change)}</span><span className={tone(stock.changePercent)}>{formatChange(stock.changePercent, true)}</span></button>) : <p className="watchlist-empty">Add a Denmark ticker to this watchlist.</p>}</div></aside><section className="chart-pane"><div className="chart-header"><div className="chart-identity"><div><h2>{selected?.code ?? "Choose a stock"}</h2><span>{companyName(selected?.code) ?? selected?.providerSymbol ?? ""}</span></div>{selected?.note && <p className="chart-note" title={selected.note}><IconNote size={16} />{selected.note}</p>}</div>{selected && <div className="quote-summary"><strong>{formatPrice(selected.last)} <small>DKK</small></strong><span className={tone(selected.change)}>{formatChange(selected.change)} · {formatChange(selected.changePercent, true)}</span></div>}<div className="timeframes">{[["1d", "1D"], ["1wk", "1W"], ["1mo", "1M"], ["4h", "4H"], ["1h", "1H"]].map(([value, label]) => <button key={value} className={timeframe === value ? "selected" : ""} onClick={() => setTimeframe(value)}>{label}</button>)}</div></div><Chart bars={bars} /><StockNote stock={selected} refresh={refreshStocks} message={setMessage} /><form className="alert-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); void api("/api/alerts", "POST", { stockId, direction: form.get("direction"), target: form.get("target") }).then(() => { event.currentTarget.reset(); return loadAlerts(); }).catch((error) => setMessage(error.message)); }}><strong>Price alert</strong><select name="direction"><option value="above">crosses above</option><option value="below">crosses below</option></select><input name="target" type="number" min="0.01" step="0.01" placeholder="DKK target" required /><button disabled={!stockId}>Create</button></form><section className="alerts"><h2>Alerts</h2>{alerts.map((alert) => <div key={alert.id}>{alert.code} {alert.direction} {alert.target.toFixed(2)} DKK · {alert.armed ? "armed" : <button className="quiet-button" onClick={() => void api(`/api/alerts/${alert.id}/rearm`, "POST").then(loadAlerts)}>Re-arm</button>}</div>)}</section></section></div>{pickerOpen && <SymbolPicker stocks={marketStocks} loading={!marketStocks.length && !marketError} error={marketError} adding={adding} onAdd={addFromPicker} onClose={() => setPickerOpen(false)} />}</main>;
}
